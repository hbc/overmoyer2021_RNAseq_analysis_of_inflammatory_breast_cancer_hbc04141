...
tpm_no <- get_counts_for_samples(samples_no, "non-pCR_expression_mean_tpm")

samples_yes <- meta %>% dplyr::filter(response == "pCR") %>% row.names() 
tpm_yes <- get_counts_for_samples(samples_yes, "pCR_expression_mean_tpm")
...
write_xlsx(list(T9.DE_response_day1 = resResponse_tb_significant), 
          "tables/T9.DE_response_day1.xlsx")
...
tpm_pos <- get_counts_for_samples(samples_pos, "Positive_expression_mean_tpm")

samples_neg <- meta %>% dplyr::filter(er == "Negative") %>% row.names() 
tpm_neg <- get_counts_for_samples(samples_neg, "Negative_expression_mean_tpm")
...
write_xlsx(list(T10.DE_ER.day1 = resER_tb_significant),
          "tables/T10.DE_ER.day1.xlsx")
...
tpm_high <- get_counts_for_samples(samples_high, "High_mean_tpm")

samples_low <- meta %>% dplyr::filter(tumor_percentage_high == "low") %>% row.names() 
tpm_low <- get_counts_for_samples(samples_low, "Low_mean_tpm")
...
resTP_tb_significant <- resTP_tb_significant %>%
    left_join(counts_tpm, by = c("gene" = "ensembl_gene_id")) %>% 
    comb_de_result_table()


write_xlsx(list(T11.DE_tumor_pct_high.day1 = resTP_tb_significant),
              "tables/T11.DE_tumor_percentage_high.day1.xlsx")
...
counts_23 <- get_counts_for_samples(samples_23, "20180323_expression_mean_tpm")

samples_28 <- meta %>% dplyr::filter(date_of == "20180228") %>% row.names() 
counts_28 <- get_counts_for_samples(samples_28, "20180228_expression_mean_tpm")

tpm_counts <- counts_28 %>%
            left_join(counts_23, 
                      by = c("ensembl_gene_id" = "ensembl_gene_id"))
...
write_xlsx(list(T12.DE_date_of.day1 = resDO_tb_significant),
           "tables/T12.DE_date_of.day1.xlsx")
...
fig4a <- ggplot(resResponse_tb_vis,
...
result_file <-  paste0("tables/day1.4gsea.txt")
...
file_conn <- file("tables/day1.4gsea.cls")
writeLines(c("22 2 1", 
