...
- patient data is paired using paired multifactor test: https://support.bioconductor.org/p/62357/#62368
...
tpm_pre <- get_counts_for_samples(samples_pre, "pre_mean_tpm" )

samples_post <- meta %>% dplyr::filter(treatment == "post") %>% row.names() 
tpm_post <- get_counts_for_samples(samples_post, "post_mean_tpm")
...
write_xlsx(list(T3.DE_treatment = resTreatment_tb_significant), 
          "tables/T3.DE_treatment.xlsx")
...
tpm_pos <- get_counts_for_samples(samples_pos, "positive_mean_tpm")

samples_neg <- meta %>% dplyr::filter(er == "Negative") %>% row.names() 
tpm_neg <- get_counts_for_samples(samples_neg, "negative_mean_tpm")
...
write_xlsx(list(T5.DE_ER = resER_tb_significant),
          "tables/T5.DE_ER.xlsx")
...
tpm_high <- get_counts_for_samples(samples_high, "tumor_high_tpm")

samples_low <- meta %>% dplyr::filter(tumor_percentage_high == "low") %>% row.names() 
tpm_low <- get_counts_for_samples(samples_low, "tumor_low_tpm")
...
write_xlsx(list(T6.DE_tumor_percentage_high = resTP_tb_significant),
          "tables/T6.DE_tumor_percentage_high.xlsx")
...
write_xlsx(list(T7.DE_date_of = resDO_tb_significant),
          "tables/T7.DE_date_of.xlsx")
